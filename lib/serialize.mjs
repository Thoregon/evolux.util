...
import { isClass }          from "./utilfns.mjs";
...
let EntityDecorator = null; // ThoregonDecorator;

let AccessObserver       = null;
let RemoteCollection     = null;
let RemoteDirectory      = null;
let RemoteFileDescriptor = null;

let getAllMethodNamesCategorized = null;

// export const useEntityDecorator = (decorator) => EntityDecorator = decorator;

export function useEntitySettings($EntityDecorator, $AccessObserver, $RemoteCollection, $RemoteDirectory, $RemoteFileDescriptor, $getAllMethodNamesCategorized) {
    EntityDecorator              = $EntityDecorator;
    AccessObserver               = $AccessObserver;
    RemoteCollection             = $RemoteCollection;
    RemoteDirectory              = $RemoteDirectory;
    RemoteFileDescriptor         = $RemoteFileDescriptor;
    getAllMethodNamesCategorized = $getAllMethodNamesCategorized;
}
...
function getAllMethodNamesCategorized(cls) {
...
export const transportserialize = (obj, opt = {}) => {
    ...
    if (isDate(obj)) return serialize(obj);
    if (!opt.excl) opt.excl = new Map();
    let id   = opt.excl.get(obj);
    if (id) return `'@ref:${id}'`;
    let type = classOrigin(obj);
...
export const persistancedeserialize = (str, opt = {}) => {
    ...
    const OptCls = opt.OptCls;
    const soul = props['@soul'];
    const typeRef    = props['@type'] ?? 'builtin:Object';
    const { Cls } = typeRef === 'builtin:Object' && OptCls ? OptCls : origin2Class(typeRef);
