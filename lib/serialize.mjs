...
export const txdeserialize = (str, opt = {}) => {
    ...
    const obj = soul
                ? ThoregonDecorator.from(soul, { Cls, typeRef, incommingSync: true })
                : AccessObserver.observe(new Cls());

    opt.refs[id] = obj;

    // skipThoregon: skip only if it does not exist locally
    if (opt.restore || !(soul != undefined && (opt.skipThoregon || !ThoregonDecorator.materialized(soul)))) {
        Object.entries(props).forEach(([prop, ser]) => {
            ...
            const value = ser?.startsWith('{') || ser?.startsWith("@ref:")
                          ...
                          : deserialize(ser);
            (soul ? ThoregonDecorator : AccessObserver).primitiveSet(obj, prop, value);
        })
    }
...
export const localdeserialize = (str, opt = {}) => {
    ...
    Object.entries(props).forEach(([prop, ser]) => {
        ...
        if (ser.startsWith('@soul:')) {
            value = ThoregonDecorator.from(ser.substring(6), { /*Cls, typeRef,*/ incommingSync: true })  // maybe add Cls to soul ref, but entity must exist
        } else {
            if (prop.startsWith('@')) return;   // skip
