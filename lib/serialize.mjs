...
export const txdeserialize = (str, opt = {}) => {
    ...
    const id = props['@id']?.substring(1).slice(0,-1);
    let ref  = opt.refs[id];
    if (ref) return ref;

    const type    = props['@type']?.substring(1).slice(0,-1) ?? 'builtin:Object';
    const { Cls } = origin2Class(type);

    const obj = Cls.$thoregonClass
                ? ThoregonDecorator.from(id, { Cls })
    ...
    Object.entries(props).forEach(([prop, ser]) => {
        ...
        const value = ser?.startsWith('{') || ser?.startsWith("'@ref:")
                        ...
                        : deserialize(ser?.substring(1).slice(0,-1));
