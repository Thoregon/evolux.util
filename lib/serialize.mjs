...
export async function decrystallize(str) {
    ...
    if (selector === 'T') {     // Thoregon Entity
        ...
        const obj = await universe.ThoregonObject.from(soul, { Cls });
        return obj;
    } if (selector === 'TO') {  // Object eventually with thoregon entities as properties
        const attrs = parts[1].split(',').map(part => part.substring(1).slice(0,-1));
        const obj = {};
        for await (const attr of attrs) {
            ...
            if (!value) {
                ...
                deserialized = await universe.ThoregonObject.from(value.substring(2));
            } else {
...
// todo [OPEN]: add also the 'classOrigin'
...
// todo [OPEN]: add also the origin to create the right instance
