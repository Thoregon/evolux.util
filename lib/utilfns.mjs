...
/**
 * check if a reference is a primitive or an object
 * @param test
 * @return {boolean}
 */
...
/**
 * give back the length (number of digits) of a number
 * @param num
 * @returns {number}
 */
export const numlength = (num) => Math.ceil(Math.log(num + 1) / Math.LN10);
...
export const aretry = /*async*/ (fn, { retries, interval } ) => {
    ...
    return new Promise(async (resolve, reject) => {
        while (!ok && retries-- > 0) {
            try {
                let result = await atimeout(() => fn(rs-retries), interval);
                ok = true;
                resolve(result);
            } catch (e) {
                if (e) {
